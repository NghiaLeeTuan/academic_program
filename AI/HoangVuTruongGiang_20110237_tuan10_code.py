...
#Code được tham khảo từ tài liệu của thầy Trần Nhật Quang - AIw10_Constraint Propagation - CSPsudoku.py
...
def flatten(seqs):
    # Hàm dùng để gộp các list con của một list cha lại với nhau
    return sum(seqs, [])

def different_values_constraint(A, a, B, b):
    # Kiểm tra xem hai ô có liên quan với nhau có giá trị đang xét giống nhau hay không
    # Trả về True nếu tụi nó khác nhau, False nếu tụi nó giống nhau
...
class CSP():
    ...
    Mô tả CSP:
    Variables: danh sách các biến của bài toán (ở đây là số ô cần điền giá trị - 81 ô)
    Domain: miền giá trị của mỗi biến - là một tập hợp biến : miền giá trị
    'Hàng xóm': các ô có liên quan với nhau, là tập hợp ô - các ô liên quan
    Constraints: ràng buộc giữa các ô liên quan là có giá trị khác nhau
    """
...
# Thiết lập một số biến để mô phỏng sudoku trên máy
_R3 = list(range(3))
...
for unit in map(set, _BOXES + _ROWS + _COLS):
    # Thêm các cô trong hàng, cột, khối vuông vào danh sách 'hàng xóm' của ô tương ứng để xét ràng buộc
    for v in unit:
...
class Sudoku(CSP):
    ...
    Một cái trạng thái ban đầu được biễu diễn bởi một dãy kí tự với 81 kí tự đại diện 81 ô
    Mỗi ô có giá trị từ 1 -> 9, những ô trống được biểu diễn bằng dấu "."
    ...
    def __init__(self, grid):
        ...
        # Giữ lại những kí tự là số và '.' biểu thị cho ô trồng, còn lại thì bỏ qua
        squares = re.findall(r'\d|\.', grid)            

        # NOTE: For variables in order of in order of 3x3 BOXES:
        # Tạo miền giá trị cho các ô trong sudoku
        # Nếu ô đã có giá trị thì miền giá trị chính là giá trị của nó
        # Nếu ô trống thì miền giá trị của nó là danh sách các số từ 1 -> 9
        ...
        # Cho biến danh sách các ô liên quan với nhau (ô: {các ô trong hàng, cột, ô vuông tương ứng})
        ...
        '''
        Khởi tạo một CSP với variables là danh sách các ô trong sudoku, miền giá trị của từng ô, 
        danh sách các ô có liên quan với nhau và constraints giữa các ô liên quan này (giá trị khác nhau)
        '''
    ...
    def display(self): # For variables in order of in order of 3x3 BOXES
        """Show a human-readable representation of the Sudoku.
            Ham giúp hiển thị sudoku một cách trực quan cho người dùng
        """
        place = 0
    ...
    def display_variables(self): # For variables in order of in order of 3x3 BOXES
        """
        Hàm hiển thị các biến đại diện cho ô trong sudoku
        """
        place = 0
        for var in self.domains.keys():
            if place%3 == 0 and place%9 != 0 :
...
def revise(csp, Xi, Xj):
    ...
    """
    Hàm này trả về giá trị True nếu như Domain được giảm bớt giá trị
    ...
    for x in csp.domains[Xi]: 
        ...
        for y in csp.curr_domains[Xj]:
            # Kiểm tra xem nếu hai biến có domain trùng nhau thì loại bớt
...
if __name__ == '__main__':       
    init_assign_easy = '..4.5....9..7346....3.21.49.35.9.48..9.....3..76.1.92.31.97.2....9182..3....6.1..'
