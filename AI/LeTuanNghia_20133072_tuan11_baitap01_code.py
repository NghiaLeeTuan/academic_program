...
def backtracking_search(csp, select_unassigned_variable=minimum_remaining_values,
    ...
    def backtrack(assignment):   
        # Ktra cac assignment deu duoc gan het gia tri roi thi tra ve assignment
        if len(assignment) == len(csp.variables):
            return assignment

        #chon ra gia tri chua duoc gan
        var = select_unassigned_variable(assignment, csp)

        #Lay gia tri tu tap domain 
        for value in order_domain_values(var, assignment, csp):  
            # Kiem tra muc do phu hop cua value voi assignment
            if csp.nconflicts(var, value, assignment) == 0:
                # Neu phu hop, value duoc gan cho var hien tai
                csp.assign(var, value, assignment)
                #Luu lai cac gia tri bi luoc bo khoi domain
                removals = [(var, a) for a in csp.curr_domains[var] if a != value] 

                csp.curr_domains[var] = [value]  #  
                if inference(csp, var, value, assignment, removals):
                    result = backtrack(assignment)
                    #neu result khac failure thi return
