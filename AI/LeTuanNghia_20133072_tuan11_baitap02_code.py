...
def min_conflicts(csp, max_steps=100000):
    #create csp(X,D,C) với max_steps là 100000(So lan thu toi da)
    """See Figure 6.8 for the algorithm"""
    csp.current = current = {} #an initial complete asiginment for csp
    #với mỗi var chọn ra một giá trị ngẫu nhiên từ các vị trí min
    for var in csp.variables:
        val = min_conflicts_value(csp, var, current)
        csp.assign(var, val, current)
    
    #cho i chạy từ 1 đến max_steps đã quy định từ trước 
    for i in range(max_steps):
        #kiểm tra conflicted trong list các biến xung đột
        conflicted = csp.conflicted_vars(current)
        #nếu không thì trả về current(solution)
        if not conflicted:
            return current
        #Nếu còn conflicted thì chọn ngẫu nhiên 1 giá trị trong tập biến xung đột
        var = random.choice(conflicted)
        #trả về giá trị cho val với mỗi var, có thể là coflicts_min hoặc là giá trị random
        val = min_conflicts_value(csp, var, current)
        #gán var = val với trạng thái current 
        csp.assign(var, val, current)
    #nếu không tìm được current(solutions) thì trả về Failure
    return None
