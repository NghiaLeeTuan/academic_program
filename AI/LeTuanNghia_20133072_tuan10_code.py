...
_R3 = list(range(3)) #tao list 0 1 2 . . . cho cac hang
_CELL = itertools.count().__next__ # tao cac truong du lieu tren map
...
for unit in map(set, _BOXES + _ROWS + _COLS):
    #them o, hang, cot vao danh sach Neighbours
    for v in unit:
...
class Sudoku(CSP):
    ...
    Moi hang` hay o duoc dien voi gia tri tu 1..9, cac hang trong hien tai duoc dien = 0 hoac '.'
...
def AC3(csp):
    ...
    ''' ADD YOUR CODE HERE '''
    while (len(queue)): # kiem tra queue khac rong
        (Xi,Xj)= queue.pop() #Remove Xi va Xj ra khoi queue
        if(revise(csp,Xi,Xj)):
            if(len(csp.domains[Xi])==0):  #kiem tra size cua domain = 0 thi tra ve fasle 
                return False
            for Xk in csp.neighbors[Xi]: #voi moi Xk trong tap neighbours cua Xi, voi moi Xk khac Xj thi ta add vao queue
                if Xk is not Xj:
                    queue.add((Xk,Xi))
