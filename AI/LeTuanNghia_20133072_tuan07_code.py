...
3. (Optional) Try other shedule() functions
...
class NQueensProblem:
    ...
    def actions(self, state):
        ...
        if state[-1] is not -1:
            ...
            col = state.index(-1) #tìm vị trị là cột có thể đặt quân hậu vào
    ...
    def value(self, node): 
        ...
        for (r1, c1) in enumerate(node.state):
            for (r2, c2) in enumerate(node.state):
                if (r1, c1) != (r2, c2):
                    num_conflicts += self.conflict(r1, c1, r2, c2) #nếu 2 quân hậu xung đột cùng 1 ô, num_conflicts +1 
...
def simulated_annealing(problem):
    ...
    #tạo node hiện tại của problem
    node=Node(problem.initial)

    #tạo vòng lặp đến vô cùng
    for i in itertools.count(start=1):
        #Tạo giá trị T
        T=schedule(i)
        #Nếu T=0, quá trình kết thuc
        if T==0:
            return node.state
        #tìm các state lân cận của state hiện tại
        neigh=node.expand(problem)
        #Nếu không tìm được đường tiếp theo
        if len(neigh)==0:
            if node.state.count(-1)>0:
                #random lại từ đầu, đề tìm solution khác
                node=Node(problem.initial)
        else:
            #chọn ngẫu nhiên 1 neigh từ node hiện tại
            next=random.choice(neigh)

            #Tính giá trị delta
            delta=problem.value(next) - problem.value(node)
            #nếu delta >0, chọn next nếu giá trị của state tiếp theo tốt hơn state hiện tại
            if delta>0:
                node=next
            #chọn với xác suất e^(delta/T)
            elif random.uniform(0.0, 1.0)< math.exp(delta/T):
                node=next
